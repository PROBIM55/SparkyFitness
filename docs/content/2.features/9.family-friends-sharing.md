...
## BudgetApp Family Sync

When SparkyFitness is opened through BudgetApp, the signed-in BudgetApp user is linked to exactly one personal SparkyFitness profile. BudgetApp also synchronizes which independently authenticated users belong to the same family account.

Family membership and health-data sharing are intentionally separate:

- A BudgetApp family connection is created in SparkyFitness with all permissions disabled.
- Joining the same BudgetApp family does not expose diaries, check-ins, reports, medications, or private settings.
- Each profile owner must explicitly enable the required permissions in the Family Access Manager.
- If two users no longer belong to the same BudgetApp family, BudgetApp-managed connections are disabled automatically.

The BudgetApp family owner is not duplicated as another family member. Their BudgetApp login, family role, member record, and SparkyFitness profile represent the same person.
